...
# Rename
old_names<-c("K6SCMON","K6SCMAX","OTHINS","PRVHLTIN","HEALTH2","IRSEX","EMPSTATY",
             "INCOME","EDUCCAT2","IREDUC2","IRMARIT","SEXRACE","NEWRACE2","CATAGE",
             "DEPNDALC","ABUSEALC","TXNEDALC","ABODALC","ALCYRTOT","BINGEHVY",
             "ALDAYPWK","ALCTRY","CIGTRY", "POVERTY2","WHODASC3","AMDELT",
             "ADPSSOC","WRKOCUP2","UADPEOP")

new_names<-c("k6month","k6max","otherins","privateins","health","gender","empstat",
             "inc","educat","educ","maristat","sexrace","race","age","alcdep",
             "alcabu","alctre","abudep","daysalc","alcbin","alcweek","alctry",
             "cigtry","pov","who","mde","feelsoc","typejob","alcsocial")
...
binh2<-binh2 %>%
  mutate(health=fct_recode(health,
                        "Good/Excellent"="(1) Excellent (HEALTH=1)",
                        "Good/Excellent"="(2) Very Good (HEALTH=2)",
                        "Good/Excellent"="(3) Good (HEALTH=3)",
                        "Fair/Poor"="(4) Fair/Poor (HEALTH=4,5)"))%>%
  filter(health!="NA")
binh2$health<-relevel(binh2$health, ref = "Fair/Poor")
...
binh2<-binh2 %>%
  mutate(alcdep=fct_recode(alcdep,
                           "No"="(0) No/Unknown (Otherwise)",
                           "Yes"="(1) Yes (See comment above)"))

# Deal with abudep
binh2<-binh2 %>%
  mutate(abudep=fct_recode(abudep,
                           "No"="(0) No/Unknown (ABUSEALC=0 and DEPNDALC=0)",
...
binh2<-binh2 %>%
  mutate(educat=fct_recode(educat,
                           "Highschool or Less"="(1) Less than high school (IREDUC2<=7 and AGE2>=7)",
                           "Highschool or Less"="(2) High school graduate (IREDUC2=8 and AGE2>=7)",
                           "Highschool & Greater"="(3) Some college (IREDUC2=9-10 and AGE2>=7)",
                           "Highschool & Greater"="(4) College graduate (IREDUC2=11 and AGE2>=7)",
                           "Highschool or Less"="(5) 12 to 17 year olds (AGE2<=6)"))

# Deal with maristat
binh2<-binh2 %>%
  mutate(maristat=fct_recode(maristat,
                             "Married"="(1) Married",
                             "Not Married"="(2) Widowed",
                             "Not Married"="(3) Divorced or Separated",
                             "Not Married"="(4) Never Been Married"))
...
binh2<-binh2 %>%
  mutate(race=fct_recode(race,
                         ...
                         "Non-white"="(2) NonHisp Black/Afr Am",
                         "Non-white"="(3) NonHisp Native Am/AK Native",
                         "Non-white"="(4) NonHisp Native HI/Other Pac Isl",
                         "Non-white"="(5) NonHisp Asian",
                         "Non-white"="(6) NonHisp more than one race",
                         "Non-white"="(7) Hispanic"))
binh2$race<-relevel(binh2$race, ref="Non-white")
binh2%>%count(race)
...
write.csv(binh2,"alcohol.csv", row.names = FALSE)
